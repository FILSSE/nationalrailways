 ...
 * the database server and the rest application for a "Sequence Diagram Cashier Ticket use case scenarion
 * that the application offers
 *
 * Author: Mihaita Tinta
 *
 *
 * flags : -s -r -n, -t -p
 */
...
mtype{ 
	...
	/* Search train route */
	Search_Train_Request,
	Search_Train_Response,
...
active proctype DataBaseServer()
{
	...
	do
		...
		::  ssocket ? Connection_Request(ChannelIn[terminalID], ChannelOut[terminalID])->
			...
			/********************************* Login User **********************************/
			/* Terminal login */
			ssocket ? Login_Request(ChannelIn[terminalID], ChannelOut[terminalID]) ->
			
			/* successfully login */
			ssocket ! Login_Response(ChannelIn[terminalID], ChannelOut[terminalID])->
			
			/********************** Customer Registration Request ***************************/
			
			/* Terminal login */
			ssocket ? Customer_Registration_Request(ChannelIn[terminalID], ChannelOut[terminalID]) ->
			
			/* successfully login */
			ssocket ! Customer_Registration_Response(ChannelIn[terminalID], ChannelOut[terminalID])->
			
			/********************** Search Train Request ***************************/
			/* Terminal login */
			ssocket ? Search_Train_Request(ChannelIn[terminalID], ChannelOut[terminalID]) ->
			
			/* successfully login */
			ssocket ! Search_Train_Response(ChannelIn[terminalID], ChannelOut[terminalID])->
...
active [terminalNumber] proctype Terminal()
{
	...
	/********************************* Login User **********************************/
	ssocket ! Login_Request(TerminalChanOut, TerminalChanIn) ->
	
	/* Wait for successfully login */
	ssocket ? Login_Response(TerminalChanOut, TerminalChanIn)->

	
	/********************** Customer Registration Request ***************************/
	ssocket ! Customer_Registration_Request(TerminalChanOut, TerminalChanIn) ->
	
	/* Wait for successfully login */
	ssocket ? Customer_Registration_Response(TerminalChanOut, TerminalChanIn)->
	
	/********************** Search Train Request ***************************/
	ssocket ! Search_Train_Request(TerminalChanOut, TerminalChanIn) ->
	
	/* Wait for successfully login */
	ssocket ? Search_Train_Response(TerminalChanOut, TerminalChanIn)->
